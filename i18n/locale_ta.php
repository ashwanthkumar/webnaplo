 ﻿<?php
	
	/**
	 *	Required or Mandatory file in the ./i18n/locale_<lang_code>.php directory where the actual locale information is added to the list
	 *
	 *	@i18n	ta
	 *	@author Janani 
	 *	@date 12/12/2011
	 **/
	 
	/**
	 * Tamil Language Script
	 **/
	$_LOCALE = array(
		// Generic Strings
		"TAMIL" => "தமிழ் ", 
		"WEBNAPLO" => "வெப்நாப்லோ ", 
		
		// Admin Page Strings
		"ADMIN" => "நிர்வாகி", 
		"HOME" => "முதற் பக்கம்", 
		"STAFF" => "பணியாளர்",
		"STUDENT" => "மாணவர்கள்",
		"LIST" => "பட்டியல்",
		"DELETE" => "நீக்கு ",
		"COURSE" => "பாடப் பிரிவு ",
		"DEPARTMENT" => "துறை ",
		"LOCK" => "பூட்டு ",
		"UNLOCK" => "திற ",
		"EDIT" => "திருத்து",
		"BLOCK" => "தடை செய்",
		"UNBLOCK" => "தடை நீக்கு ",
		"USERS" => "உபயோகிப்பவர்கள்", 
		"BLOCK_UNBLOCK_USERS" => "உபயோகிப்பவர்களை தடை செய் / நீக்கு ",
		"LOGOUT" => "மூடு ",
		"SETTINGS" => "அமைப்பு ",
		"ADVANCED_SETTINGS" => "முற்றிய அமைப்பு ",
		"SASTRA_UNIVERSITY" => "சாஸ்த்ரா பல்கலைக்கழகம் ",
		"CHOOSE_ACTION" => "தேர்வு செய் ",
		"APPLY_SELECTED" => " பயன்படுத்து ",
		"STAFF_LIST" => "பணியாளர் பட்டியல்",
		"COURSE_LIST" => "பாடப் பிரிவு பட்டியல்",
		"PROGRAMME_LIST" => "பட்டப்பிரிவு பட்டியல்",
		"PROGRAMME" => "பட்டப்பிரிவு ",
		"NEWS_UPDATES" => "தலைப்புச்செய்திகள்",
		"CAMPUS_STATUS" => "வளாக நிலமை  ",
		"DEPARTMENT" => "துறை   ",
		"SYSTEM_STATUS" => "மென்பொருள் நிலமை ",
		"VERSION" => "பதிப்பு  ",
		"RELEASE" => "வெளியீடு பெயர்",
		"BUILD_DATE" => "வெளியீடு நாள்",
		"STATUS" => "நிலமை",
		"AND" => "மற்றும் ",
		"ADVANCED_IMPORT_SETTINGS"  => "இறக்குமதி",
		"DELETE_PROGRAMME" => "பட்டப்பிரிவை நீக்கு ",
		"DELETE_COURSE" =>"பாடப்பிரிவை நீக்கு",
		"DELETE_DEPARMENT"=>"துறையை நீக்கு",
		"DELETE_SECTION" => "பிரிவை நீக்கு",
		"DELETE_STAFF" => "பணியாளரை நீக்கு ",
		"DELETE_STUDENT"=>"மாணவரை நீக்கு",
		"CHANGEDAYORDER_TITLE" => "மாற்ற  வேண்டிய நாள் ",
		"DELETE_SELECTED" => "தேர்ந்தெடுக்கப்பட்டதை  நீக்கு",
		"ADVANCED_CHANGE_DAY_ORDER" => "மாற்ற  வேண்டிய நாட்கள்",

		// Dataentry
		"ADD" =>"சேர்க்க",
		"ADD_PROGRAMME"=>"பட்டப்பிரிவை சேர்க்க",
		"ADD_COURSE"=>"பாடப்பிரிவை  சேர்க்க",       
		"COURSE_CODE" => "பாடப்பிரிவு எண் ",
		"COURSE_NAME" =>"பாடப்பிரிவு பெயர் ",
		"CREDITS" => "மதிப்பீடு",
		"PROGRAMME_NAME" => "பட்டபிரிவின் பெயர் ",
		"RESET" => "மாற்ற",
		"ADD_DEPARMENT"=>"துறை  சேர்க்க ",
		"DEPARTMENT_NAME" =>"துறையின் பெயர்",
		"NAME" => "பெயர்",
		"ADD_STUDENT"=>"மாணவனைச் சேர்க்க",
		"REGISTRAION_NO" => "சேர்க்கை எண் ",
		"SECION_NAME" => "பிரிவின் பெயர்",
		"YEAR"=>"வருடம்",
		"ADD_STAFF"=> "பணியாளரைச் சேர்க்க",
		"DESIGNATION" => "பதவி",
		"STAFF_ID" => "பணியாளர் எண் ",
		"EMAIL_ID" => "மின்னஞ்சல் முகவரி",
		"MOBILE_NO"=>"அலைப்பேசி எண் ",
		"ADDRESS" => "முகவரி",
		"ADD_SECTION" => "பிரிவைச் சேர்க்க",
		"QUICK_STATS" => "விரைவு விவரம்",
		"CHANGE_PASSWORD" => "மாற்று மறைச்சொல் ",
		"NEW_PASSWORD" =>"புதிய மறைச்சொல்",
		"UPDATE_PASSWORD" => "மறைச்சொல்லை புதுப்பித்தல்",

		// Staff Module 
		"COURSE_PROFILE" => "பாடப்பிரிவு குறிப்பு",
		"ATTENDANCE" => "வருகைப் பதிவேடு ",
		"PENDING_ATTENDENCE" => "நிலுவையிலுள்ள நாட்கள்",
		"CIA" =>"மதிப்பெண் பட்டியல்",	// @TODO Is this correct? Is it not CIA List? We want just CIA!
		"_CIA" =>"சிஐஎ",	
		"VIEW" =>"பார்",
		"PROFILE"=> "சிறப்புக் குறிப்பு",
		"STAFF_PROFILE" => "பணியாளர் சிறப்புக் குறிப்பு  ",
		"CUMULATIVE_REPORT" => " ஒன்று சேர்ந்த அறிக்கை ",
		"MARK_TYPE" => "மதிப்பெண் வகை", 
		"TOOLS" => "டூல்ஸ்",
		"POST_MARKS" => "மதிப்பெண்களை  குறிப்பிடு ",
		"ASSIGNMENT" => "பணி ",
		"STUDENT_CONFIRMATION" => "மதிப்பெண்களை உறுதிசெய்",
		"TOTAL" => "மொத்தம்",
		"CONFIRM" => "உறுதிசெய்",
		
		// Student Page
		"FEEDBACK"=> "கருத்து",
		"CALENDAR" => "ஆண்டு விவரக் குறிப்பு",
		"TIMETABLE" =>	"கால அட்டவனண",
		"MONDAY" => "திங்கட்கிழமை ",
		"TUESDAY" => "செவ்வாய்க்கிழமை ",
		"WEDNESDAY" => "புதன்கிழமை",
		"THURSDAY" => "வியாழக்கிழமை ",
		"FRIDAY" => "வெள்ளிக்கிழமை",
		"TIMESLOTS" => "காலகட்டம் ",
		"DAYS" => "நாட்கள்", 
		"STUDENT_CIA_MARKS" => "மாணவர்களின் சிஐஎ மதிப்பெண்கள்",
		"HOURS_PRESENT" => "வந்திருக்கிற நேரம் ",
		"HOURS_TOTAL" => "மொத்த  நேரம்",
		"PERCENTAGE" => "சதவீதம்",
		
		// Login Page Strings
		"LOGIN" => "நுழை ",
		"USERNAME" => "பயனீட்டாளர் பெயர்",
		"PASSWORD" => "மறைச்சொல்",
		"LOGIN_PAGE" => "நுழைவு வாயில் ",
		"WELCOME_TO_WEBNAPLO" => "வெப்நாப்லோ உங்களை வரவேற்கிறது",
		"ENTER_USERNAME" => "பயனீட்டாளர் பெயர் எழுது  ",
		"ENTER_PASSWORD" => "மறைச்சொல் எழுது  ",
	);
	

	// Add the locale information to the system
	locale('ta', $_LOCALE);
